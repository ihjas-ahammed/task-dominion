...
class AIGenerationActions {
  ...
  Future<void> generateGameContent(int levelForContent,
      {bool isManual = false, bool isInitial = false}) async {
    ...
    print(
        "[AIActions] Starting generateGameContent for level $levelForContent. Manual: $isManual, Initial: $isInitial");
    ...
    final existingEnemyIdsString =
        _provider.enemyTemplatesList.map((e) => e.id).join(', ');
    final existingArtifactIdsString =
        _provider.artifactTemplatesList.map((a) => a.id).join(', ');
    final existingLocationIdsString =
        _provider.gameLocationsList.map((loc) => loc.id).join(', '); // Added
    ...
    try {
      final result = await _aiService.generateGameContent(
        ...
        existingEnemyIdsString:
            existingEnemyIdsString.isNotEmpty ? existingEnemyIdsString : 'none',
        existingArtifactIdsString: existingArtifactIdsString.isNotEmpty
            ? existingArtifactIdsString
            : 'none',
        existingLocationIdsString: existingLocationIdsString.isNotEmpty
            ? existingLocationIdsString
            : 'none', // Added
        themes: themes,
        onLog: (logMessage) {
      ...
      final List<EnemyTemplate> uniqueNewEnemies = result['newEnemies']!
          ...
          .where(
              (e) => !_provider.enemyTemplatesList.any((et) => et.id == e.id))
          .toList();

      final List<ArtifactTemplate> uniqueNewArtifacts = result['newArtifacts']!
          .map((aJson) => ArtifactTemplate.fromJson(aJson))
          .where((a) =>
              !_provider.artifactTemplatesList.any((at) => at.id == a.id))
          .toList();

      final List<GameLocation> uniqueNewLocations =
          (result['newGameLocations'] ?? [])
              .map((locJson) => GameLocation.fromJson(locJson))
              .where((loc) =>
                  !_provider.gameLocationsList.any((gl) => gl.id == loc.id))
              .toList();

      print(
          "[AIActions] AI generated: ${uniqueNewEnemies.length} enemies, ${uniqueNewArtifacts.length} artifacts, ${uniqueNewLocations.length} locations.");

      _provider.setProviderState(
          enemyTemplatesList: [
            ..._provider.enemyTemplatesList,
            ...uniqueNewEnemies
          ],
          ...
          currentGame: CurrentGame(
            ...
            log: [
              ...
              "<span style=\"color:${AppTheme.fhAccentGreen.value.toRadixString(16).substring(2)};\">New challenges, treasures, and realms have appeared! (AI Generated)</span>"
            ],
            currentPlaceKey: _provider.currentGame.currentPlaceKey,
          ));
    } catch (e) {
      ...
      _provider.setProviderState(
        ...
        log: [
          ..._provider.currentGame.log,
          "<span style=\"color:${AppTheme.fhAccentRed.value.toRadixString(16).substring(2)};\">AI content generation failed: ${e.toString()}</span>"
        ],
        currentPlaceKey: _provider.currentGame.currentPlaceKey,
      ));
    } finally {
  ...
  Future<void> triggerAISubquestGeneration(MainTask mainTaskForSubquests,
    ...
    if (_provider.isGeneratingSubquests) {
      print(
          "[AIActions] triggerAISubquestGeneration skipped, already in progress.");
      return;
    }
    _provider.setProviderAISubquestLoading(true);
    print(
        "[AIActions] Starting triggerAISubquestGeneration for task '${mainTaskForSubquests.name}'. Mode: $generationMode");
    ...
    try {
      final generatedSubquestsRaw = await _aiService.generateAISubquests(
        ...
        onNewApiKeyIndex: (newIndex) {
          print(
              "[AIActions] API key index updated to $newIndex during subquest gen.");
          _provider.setProviderApiKeyIndex(newIndex);
        },
        onLog: (logMessage) {
          _provider.setProviderState(
      ...
      print(
          "[AIActions] Generated ${newSubTasksForParent.length} new sub-quests for task '${mainTaskForSubquests.name}'.");
      ...
      _provider.setProviderState(
          ...
          currentGame: CurrentGame(
            ...
            log: [
              ..._provider.currentGame.log,
              "<span style=\"color:${AppTheme.fhAccentGreen.value.toRadixString(16).substring(2)};\">AI successfully generated ${generatedSubquestsRaw.length} new sub-quests for '${mainTaskForSubquests.name}'.</span>"
            ],
            currentPlaceKey: _provider.currentGame.currentPlaceKey,
          ));
    } catch (e) {
      print(
          "[AIActions] Error in triggerAISubquestGeneration: ${e.toString()}");
      _provider.setProviderState(
          currentGame: CurrentGame(
        enemy: _provider.currentGame.enemy,
        playerCurrentHp: _provider.currentGame.playerCurrentHp,
        log: [
          ..._provider.currentGame.log,
          "<span style=\"color:${AppTheme.fhAccentRed.value.toRadixString(16).substring(2)};\">AI sub-quest generation failed: ${e.toString()}</span>"
        ],
        currentPlaceKey: _provider.currentGame.currentPlaceKey,
      ));
    } finally {
